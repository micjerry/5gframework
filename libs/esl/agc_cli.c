...
int main(int argc, char *argv[])
{
	...
	if ((ccheck = getenv("AGC_CLI_COLOR"))) {
		is_color = esl_true(ccheck);
	}

	strncpy(internal_profile.host, "127.0.0.1", sizeof(internal_profile.host));
	strncpy(internal_profile.pass, "Nouse", sizeof(internal_profile.pass));
	strncpy(internal_profile.name, "internal", sizeof(internal_profile.name));
	internal_profile.port = 6521;
	...
	if (argv_host) {
		if (argv_port && profile->port != 8051) {
			snprintf(bare_prompt_str, sizeof(bare_prompt_str), "agc@%s:%u@%s> ", profile->host, profile->port, profile->name);
		} else {
			snprintf(bare_prompt_str, sizeof(bare_prompt_str), "agc@%s@%s> ", profile->host, profile->name);
		}
	} else {
		snprintf(bare_prompt_str, sizeof(bare_prompt_str), "agc@%s> ", profile->name);
	}
 ...
 connect:
	...
	while (--loops > 0) {
		...
		if (esl_connect_timeout(&handle, profile->host, profile->port, connect_timeout)) {
